...
func CreateIdP(idp model.IdP) error {
	...
	_, err = dbClient.Execute(QueryCreateIdP, idp.ID, idp.Name, idp.Description, idp.ClientID, idp.ClientSecret, idp.RedirectURI, string(scopes))
	if err != nil {
...
func UpdateIdP(idp *model.IdP) error {
	...
	rowsAffected, err := dbClient.Execute(QueryUpdateIdPByIdPID, idp.ID, idp.Name, idp.Description, idp.ClientID, idp.ClientSecret, idp.RedirectURI, string(scopes))
	if err != nil {
		logger.Error("Failed to execute query", log.Error(err))
		return fmt.Errorf("failed to execute query: %w", err)
	}
